...
.
.P
Zombie also supports promises\.  When you call functions like \fBvisit\fR, \fBwait\fR or \fBclickLink\fR without a callback, you get a
...
get additional and quite useful extensions, such as \fB:not(selector)\fR, \fB[NAME!=VALUE]\fR, \fB:contains(TEXT)\fR, \fB:first/:last\fR and so forth\.  Check out the Sizzle\.js documentation \fIhttps://github\.com/jeresig/sizzle/wiki\fR for more
details\.
.
.P
...
first argument can be a CSS selector (see \fIHunting\fR), the \fBA\fR element, or the
...
returned by \fBpressButton\fR\|\.  You can use this pattern whenever you need to wait,
after \fBvisit\fR, \fBreload\fR, \fBclickLink\fR, etc\.
.
.P
...
functions with an empty \fBreturn\fR\|\.
.
.P
...
.
.P
\fBNote:\fR This usage of \fBbind\fR is one way to allow a function defined in some
other context to use the \fBBrowser\fR object available in this context\.
.
.P
Let\'s talk a bit about error handling\.  In promise\-land, an error causes the
promise to be Qrejected with that \fBError\fR object as an argument\.  However,
...
with do that by calling \fBthen\fR with the same callback for both resolved and
rejected promises\.
.
.P
...
.
.P
This is particular to Mocha\.  If we used \fBthen(done)\fR, the callback would never
get called since the promise gets rejected (no such page to load)\.  If we used \fBfail(done)\fR the callback would be called with an error, causing the test to
fail\.  But \fBfinally\fR calls our callback whether the promise got resolved or
rejected\.
.
.P
